...
**Balanced:** “Owns the worker realm and its host bridge. Realm initialization is single-shot; disposal terminates the worker and rejects later calls. See the owning ADR for the protocol rationale.”
...
**Over-trimmed:** Deleting the entire Testing section because the decision record has already shipped.
...
**Balanced:** “Provider selection is cached for the plugin lifetime; installing or repairing a provider requires reload.” Keep ordinary cleanup in its TODO or owning decision record.
