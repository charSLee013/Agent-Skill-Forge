...
- Add `--hidden --glob '!.git/**'` when the scope includes dot-directories; ripgrep skips them by default and local agent workspace prose is easy to miss.
- Exclusions go last so a later include cannot re-admit them: `--glob '!vendor/**' --glob '!node_modules/**' --glob '!skills/productivity/trim-cot-leakage/**'`, plus recorded fixture and snapshot directories in scope. This skill's files quote leaked wording as calibration; judge those self-hits as evidence, not usage.
