...
**Fixed:** "Slash input resolves against the visible catalog — the rule is owned by the committed ADR or issue that records it."
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and its owning decision record.
...
An unaided pass deleted this, reasoning that issue citations belong only in decision records. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. Durable decision records and postmortems may additionally cite *merged PRs* as evidence.
...
**Original:** "The gate notice narrates the check order; the notice text is also what the documentation check compiles against."

**Overcorrected:** "…" (whole sentence deleted as narration.)

**Right:** "The notice text is what the documentation check compiles against."
