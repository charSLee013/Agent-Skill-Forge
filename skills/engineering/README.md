...
- [setup-agent-skills](./setup-agent-skills/SKILL.md) — configures `.codex/agents/`, triage labels, domain docs, and the root instruction shape; use it when a hard-dependency engineering skill needs the local workspace, not as a universal feature-work prerequisite; it requires repository write access and exits with a ready local workspace.
