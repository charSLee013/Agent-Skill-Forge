---
name: triage
description: Move local issues through a state machine of triage roles — categorise, verify, grill if needed, and write agent-ready briefs.
...
- "Let's look at `.codex/agents/work/search/issues/01-index-errors.md`"
- "Move `.codex/agents/work/search/issues/01-index-errors.md` to ready-for-agent"
...
5. **Apply the outcome:**
   ...
   - `needs-info` — add triage notes (template below).
   - `wontfix` — mark closed, with the note depending on *why*:
     - **Already implemented** — the change already exists in the codebase. Point to where it lives; do **not** write to `.out-of-scope/` (that KB is for *rejected* requests, not built ones).
     - **Rejected (bug)** — polite explanation, then mark closed.
     - **Rejected (enhancement)** — write to `.out-of-scope/`, link to it from a note, then mark closed ([OUT-OF-SCOPE.md](OUT-OF-SCOPE.md)).
   - `needs-triage` — apply the role. Optional comment if there's partial progress.

## Quick state override

If the maintainer says "move this issue to ready-for-agent", trust them and apply the role directly. Confirm what you're about to do (role changes, note, close), then act. Skip grilling. If moving to `ready-for-agent` without a grilling session, ask whether they want to write an agent brief.
