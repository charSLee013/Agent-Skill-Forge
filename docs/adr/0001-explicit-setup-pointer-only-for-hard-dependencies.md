# Explicit `/setup-agent-skills` pointer only for hard dependencies

Engineering skills depend on per-repo config (local issue workspace, triage label vocabulary, domain doc layout) seeded by `/setup-agent-skills`. Some skills cannot meaningfully function without that config — they have to publish to the repo's `.codex/agents/` workspace or apply a specific label string. Others only use it to sharpen output (vocabulary, ADR awareness) and degrade gracefully without it.
